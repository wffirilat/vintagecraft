...
public class BlockStonecutter extends BlockContainerVC {
	...
	public void getSubBlocks(Item itemIn, CreativeTabs tab, List list) {
		for (EnumTree treetype : EnumTree.values()) {
			if (treetype.jankahardness > 800) {
				list.add(((ItemStonecutter)itemIn).withTreeType(treetype));
			}
		}
	}
	...
	public EnumTree getTreeType(IBlockAccess worldIn, BlockPos pos) {
		TEStonecutter te = (TEStonecutter) worldIn.getTileEntity(pos);
		if (te != null) {
			return te.treeType;
		}
		return null;
	...
	public ItemStack getItemStackFor(EnumTree treeType) {
		return ((ItemStonecutter)Item.getItemFromBlock(this)).withTreeType(treeType);
	}
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumFacing side, float hitX, float hitY, float hitZ) {
		playerIn.openGui(VintageCraft.instance, 7, world, pos.getX(), pos.getY(), pos.getZ());
		return true;
	}
	...
	public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
        ...
        if (tileentity instanceof TEStonecutter) {
        	((TEStonecutter) tileentity).ejectContents();
        }
